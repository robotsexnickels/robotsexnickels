...
uint256 const hashGenesisBlock("0xb1346c989e2a50eec5af9db51bee21162de767bc14e90dfc4c6118d47d97976a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // RobotSexNickels: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "RobotSexNickels Signed Message:\n";
...
{
    // RobotSexNickels: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // Our algo is: (abs(sin((nHeight+600)/200))*500000.0/(nHeight+600))+(1.0/(nHeight+600))
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // RobotSexNickels: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // RobotSexNickels: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "SFGate 10/Jul/2014 Call girl tied to dead rocker";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1405577924;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3759455;

        if (fTestNet)
        {
            block.nTime    = 1405047600;
            block.nNonce   = 3051817;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x11078ce1e1b919bf4f6bcabb7efca72839abb568398a034b6eb8036c43128ba3"));
...
unsigned char pchMessageStart[4] = { 0xf9, 0xe8, 0xb4, 0xda }; // RobotSexNickels
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RobotSexNickelsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RobotSexNickelsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RobotSexNickelsMiner(CWallet *pwallet)
{
    printf("RobotSexNickelsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("robotsexnickels-miner");
    ...
    try { loop {
        ...
        printf("Running RobotSexNickelsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RobotSexNickelsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RobotSexNickelsMiner, pwallet));
}
