...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("RobotSexNickels version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  robotsexnickelsd [options]                     " + "\n" +
                  "  robotsexnickelsd [options] <command> [params]  " + _("Send command to -server or robotsexnickelsd") + "\n" +
                  "  robotsexnickelsd [options] help                " + _("List commands") + "\n" +
                  "  robotsexnickelsd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "robotsexnickels:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: robotsexnickels.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: robotsexnickelsd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 5556 or testnet: 55556)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 5555 or testnet: 55555)") + "\n" +
        ...
        "\n" + _("SSL options: (see the RobotSexNickels Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. RobotSexNickels is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("RobotSexNickels version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "RobotSexNickels server starting\n");
    ...
    CService addrOnion;
    unsigned short const onion_port = 9069;
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of RobotSexNickels") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart RobotSexNickels to complete") << "\n";
